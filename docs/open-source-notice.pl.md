# Informacje o licencjach open source

## Krótka wersja do aplikacji

Ta aplikacja korzysta z silnika szachowego Stockfish.

Stockfish jest projektem open source udostępnianym na licencji GPL-3.0.

Kod źródłowy Stockfisha:
https://github.com/official-stockfish/Stockfish

Tekst licencji Stockfisha:
https://github.com/official-stockfish/Stockfish/blob/master/Copying.txt

Kod źródłowy tej aplikacji:
TU_WPISZ_LINK_DO_REPO_APLIKACJI

## Dłuższa wersja do ekranu „O aplikacji” lub strony projektu

Ta aplikacja korzysta z lokalnego silnika szachowego Stockfish.

Stockfish jest wolnym i otwartym oprogramowaniem udostępnianym na licencji GNU General Public License v3.0 (GPL-3.0).

Projekt Stockfish:
https://github.com/official-stockfish/Stockfish

Licencja Stockfisha:
https://github.com/official-stockfish/Stockfish/blob/master/Copying.txt

Kod źródłowy tej aplikacji oraz informacje o użytej wersji Stockfisha:
TU_WPISZ_LINK_DO_REPO_APLIKACJI
